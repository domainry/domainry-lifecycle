...
func TestAllDataScopeAddsNoRangePredicate(t *testing.T) {
	...
	builder := query.NewWorkspaceSelectBuilder(renderer.WithSchema(""), "_lifecycle_legal_holds", "workspace-a").Columns("payload_json")
	...
	if store.cleanupJobScopePredicate("workspace-a", lifecyclepersistence.UnrestrictedDataScopeFilter()) != nil {
...
func TestBoundedDataScopeRendersOwnerAndOrganizationInSQL(t *testing.T) {
	...
	statement, args, err := query.NewWorkspaceSelectBuilder(renderer.WithSchema(""), "_subject_requests", "workspace-a").Columns("payload_json").
